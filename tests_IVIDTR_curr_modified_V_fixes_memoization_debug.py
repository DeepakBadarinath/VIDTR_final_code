...
import IVIDTR_curr_modified_V_fixes_memoization_debug as VIDTR_module
...
from IVIDTR_curr_modified_V_fixes_memoization_debug import VIDTR
...
#%%    
    ...
    def test_last_timestep_optimal():
        ...
        assert val_func(np.array([0,0])) == reward_coeff / (f+1)
        
        assert val_func(np.array([0,1])) == reward_coeff 
        assert optimal_policy(np.array([0,1])) == np.array([0,-1])
        
        assert val_func(np.array([-2,0])) == reward_coeff / (f+1)
        assert optimal_policy(np.array([-2,0])) == np.array([1,0])
    
        assert val_func(np.array([-3,-3])) == reward_coeff / ( f*3.605551275463989 + 1 )
        assert optimal_policy(np.array([-3,-3])) in [np.array([0,1]), np.array([1,0])]
   ...
   #%%%
    ...
    def test_last_step_int_bellman():
        ...
        s = np.array([0,0])
        a = np.array([0,1])
        assert last_step_bellman_I(s,a) == grid_class.algo.MDP.reward_functions[t](s,a)
   ...
   #%%%
    ...
    error_file_name = f'vidtr_errors.csv'
   
    optimal_conditions, optimal_actions = grid_class.algo.compute_interpretable_policies(integration_method='static_integral',
  ...
  #%%
   ...
   #%%%
    ...
    integration_methods = ['integrate_static', 'trajectory_integrate']
    random_seeds = [10, 100, 1000]
    conditions_strings = ['all', 'order_stats']
    Ns = [500, 1000, 2000, 5000]
    
    for i, int_method in enumerate(integration_methods):
        for i, r in enumerate(random_seeds):
            for i, cs in enumerate(conditions_strings):
                for i, N in enumerate(Ns):
            
                    obs_states, obs_actions, obs_rewards = grid_class.generate_random_trajectories(N, random_seed=r)
                    
                    error_file_name = f'Int_method_{int_method}_rs_{r}_cs_{cs}_traj_no_{N}.csv'
                    
                    optimal_conditions, optimal_actions = grid_class.algo.compute_interpretable_policies(integration_method=int_method,
                                                                                                         conditions_string = cs,
                                                                                                         obs_states = obs_states,
                                                                                                         store_errors=True,
                                                                                                         error_file_name = error_file_name) 
                   ...
                   #%%
                    ...
                    for t in range(grid_class.time_horizon):                                   
                        ...
                        grid_class.env.plot_policy_2D(int_policy, title=f'Int. policy at time {t}',
                                                      saved_fig_name = f'vidtr_time_{t}_cs_{cs}_r_{r}_int_method_{int_method}')
